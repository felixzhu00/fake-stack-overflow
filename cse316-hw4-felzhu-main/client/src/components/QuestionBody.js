...
export default class QuestionBody extends React.Component {
  ...
  handleClick(e) {
    axios.get(`http://localhost:8000/incrementView/${this.props.questions[e.target.id].qid}`).then(res => {
      this.props.change_page("Question:" + this.props.questions[e.target.id].qid)
    })
  ...
  render() {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      let element = (
        <tr key={questions[i].qid}>
          <td className="e1">
            <ul>
              <li>{questions[i].views}</li>
              <li>{questions[i].answer_length}</li>
            </ul>
          </td>
          <td className="e2">
            <ul>
          ...
          <td className="e3">
            <ul>
              ...
              <EmTag key={questions[i].qid} questionNum={questions[i].qid}/>
            </ul>
          </td>
          <td className="e4">
            <ul>
